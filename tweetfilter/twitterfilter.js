...
Articles = new Mongo.Collection("tweets");
...
if (Meteor.isClient) { 
  ...
  Template.leaderboard.helpers({
    ...
    selectedArticle: function () {
      var tweet = Articles.findOne(Session.get("selectedArticle"));
  ...
  Template.leaderboard.events({
    ...
    'click .yes': function () {
      ...
      tweet_title = Articles.find( {_id:Session.get("selectedArticle")}).map(function(x) { return x.title;});
      tweet_conf = Articles.find( {_id:Session.get("selectedArticle")}, {fields: {'confidence': 1}}).map(function(x) {return x.confidence;}); 
